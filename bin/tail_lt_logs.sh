...
journalctl -u lambdatest -f -n "${LINES_TO_SHOW}" | grep -E 'Monitor|a55-perf' | \
        GREP_COLOR='1;32' grep --color=always -E '.*Launched.*|^' | \
        GREP_COLOR='1;35' grep --color=always -E '.*LT Monitor.*|^' | \
        GREP_COLOR='1;34' grep --color=always -E '.*TC Monitor.*|^' | \
        GREP_COLOR='1;31' grep --color=always -E '.*Direct.*|^'
