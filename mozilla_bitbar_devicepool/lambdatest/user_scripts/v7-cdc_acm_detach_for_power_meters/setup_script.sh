...
# python3 now comes from the runtime parameter in the hyperexecute.yaml
...
# The AVHzy CT-3 power meter (VID 0x0483) presents as CDC-ACM, so the cdc_acm
# kernel module claims its interfaces at container start before usb-power-profiling
# can open them via libusb (EBUSY). Unbind cdc_acm from every 0x0483 interface so
# libusb gets a clean device. Must run on every job: cdc_acm re-claims the device
